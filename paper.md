...
1. **D has the highest mean mIoU**, by +0.40 / +0.59 / +0.46 over A / B / C. We report both significance tests side by side — they probe two different variance sources (image sampling vs training seed), so neither alone settles every pair.
   * **Primary — paired image-bootstrap (n = 500, B = 10 000, Holm):** **D > B is significant** (Δ = +0.59, Holm p = 0.032) and **D > A is significant** (Δ = +0.40, Holm p = 0.043); **D > C is not significant after Holm** (Δ = +0.46, raw p = 0.023, Holm p = 0.092). A, B and C are mutually non-significant.
   * **Seed robustness — paired-by-seed t-test (n = 3, Holm):** **D > C is significant** (Δ = +0.46, t = 11.8, raw p = 0.007, Holm p = 0.042 — C has very low inter-seed variance); **D > A (p = 0.095) and D > B (p = 0.075) are not** (underpowered at three seeds, though all three seeds favour D in both cases).
   * **Synthesis:** D has the highest mIoU, and every pairwise "D > {A, B, C}" is supported by at least one of the two tests — the image test establishes D > A and D > B, the seed test establishes D > C. The two tests probe different variance sources (resampling the 500-image evaluation set vs re-drawing the training seed), so we report both rather than cherry-pick the one that flatters a given pair. The v0.1.0 claim that D's lower CI bound clears the others' upper bounds relied on 1.96-scaled (too-narrow) seed intervals; the honest statement is that *D is the best-mean recipe, its lead over the field-standard baseline B and over plain CE (A) is significant under the primary image-bootstrap, and its lead over the joint variant C is significant under the seed-level test*.
...
**Which pairing is the BRATS analogue?** In our BRATS work the veto is the **field-standard baseline** (`DC_and_CE`, i.e. Dice + CE — the default nnU-Net loss), and the generalist it prunes (DistMap) *also keeps Dice + CE*. The faithful Cityscapes transcription must therefore use the **CE + Dice baseline** as the veto. Among our four variants that baseline is **B** (CE + Dice) — it is the field-standard segmentation baseline and equals nnU-Net's default `DC_and_CE` loss used by the BRATS work; A (CE-only) and D (CE + Kervadec, no Dice) are Cityscape-specific *Dice-axis* ablation points with **no BRATS equivalent** (nnU-Net always includes Dice). The tight analogue of the BRATS rule is thus the boundary-with-Dice variant **C** (which, like DistMap, keeps Dice + CE) vetoed by the Dice + CE baseline **B**: the **canonical consensus is C⊘B**. (The D⊘B pairing reported in v0.2.1 is a Cityscape-specific 2D finding — its veto B is the baseline, but its generalist D drops Dice, so it is *not* the direct BRATS transcription; we report it below as a contrast.)
...
|---|---|---|---|
| **C⊘B (canonical)** | 81.24 | +0.010 pp | **−18.7 %** |
| D⊘B (v0.2.1 finding) | 81.65 | −0.033 pp | −18.6 % |
| C⊘A | 81.31 | +0.085 pp | −8.5 % |
| D⊘A | 81.82 | +0.137 pp | −15.6 % |

Two structural facts. First, **every pairing is mIoU-neutral** (|Δ mIoU| ≤ 0.14 pp): the veto prunes fragments without disturbing region overlap, by design. Second, **the veto B (= Dice + CE baseline) prunes ≈ 2× more fragments than the veto A** (C⊘B −18.7 % vs C⊘A −8.5 %; D⊘B −18.6 % vs D⊘A −15.6 %): the Dice baseline B is a stronger, cleaner veto mask, which confirms that the BRATS-faithful veto is the baseline B rather than the CE-only ablation A.

**Canonical consensus C⊘B (full characterisation, fused vs primary C, paired n = 3).** This is the tight BRATS analogue (a Dice + CE generalist vetoed by the Dice + CE baseline). Against the primary C it is:

| Metric | Δ (fused − C) | p (seed paired-t, n = 3) | image-bootstrap |
|---|---|---|---|
| mIoU | +0.010 pp | 0.747 | Δ = −0.01, p = 0.86 → **neutral** |
| fragments | −18.7 % (−111/img) | 0.007 | — |
| Boundary F1 | −0.104 pp | 0.009 | negligible (< 0.1 pp) |
| Trimap IoU | +0.058 pp | 0.028 | negligible (< 0.1 pp) |

The mIoU is confirmed neutral by *both* the seed-level paired-t (p = 0.747) and the paired image-bootstrap (Δ = −0.01, p = 0.86). The Boundary F1 and Trimap IoU shifts are statistically detectable at n = 3 (p = 0.009 and 0.028) but **practically negligible** — both under 0.1 pp. So C⊘B is a **pure spatial-coherence cleanup**: it removes ≈ 19 % of connected-component fragments at **zero mIoU cost and zero meaningful boundary-quality cost**. This is the clean BRATS analogue — region-overlap-neutral, the gain living entirely on fragmentation, a property to which dataset-level mIoU is blind (which is exactly why the fragment-count metric is reported alongside it; on BRATS the analogous rule was Dice-neutral but improved boundary HD95).

**D⊘B contrast (the v0.2.1 finding).** D is the best single variant, so D⊘B is a natural pairing, but its veto B is the baseline while its generalist D drops Dice — so it is not the direct BRATS transcription. It gives the **same prune and the same mIoU-neutrality** (fragments −18.6 %, −146/img, p = 0.007; mIoU Δ = −0.033 pp, seed p = 0.50, image-bootstrap Δ = +0.03, p = 0.66) **but a notable Dice-character shift**: Boundary F1 −0.691 pp and Trimap IoU +0.997 pp (both p < 0.001). The mechanism: reassigning D's fragments to B's labels *Dice-ifies* the output, because D (no Dice) and B (Dice) sit on opposite ends of the Dice axis (§5.1) — the fused mask inherits B's region-coherent, contour-softened character on the reassigned pixels. C⊘B (both variants on the Dice side) has no such side-effect. **C⊘B is therefore the *pure* consensus; D⊘B conflates the fragment cleanup with a Dice-axis shift.**
...
* **Consensus fully characterised; n = 3 caveat stands.** §5.5 now reports the variant-pair veto's effect on all four metrics (mIoU, fragments, Boundary F1, Trimap — the BRATS HD95 analogue) for the full pairing matrix: the canonical C⊘B is boundary-quality-neutral (Boundary F1 / Trimap shifts < 0.1 pp), whereas D⊘B carries a measurable Dice-axis shift (Boundary F1 −0.69 pp, Trimap +1.00 pp). The mIoU-neutrality is confirmed by both the image-bootstrap and the seed-level test; the contour deltas are detectable but, being measured at three seeds, share the same low-power caveat as the rest of the seed-level analysis.
