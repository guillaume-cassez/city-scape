...
def parse_variant_seed(path_str: str):
    """Parse ('A', 42) from a full checkpoint path; works for both the original
    layout (.../pilot_fullres_A_ce_seed42/epoch_160.pth) and the slimmed layout
    (.../reeval_slim/pilot_fullres_A_ce_seed42__epoch_160.pth)."""
    v = re.search(r"_([ABCD])_", path_str)
    s = re.search(r"seed(\d+)", path_str)
    return (v.group(1) if v else "?"), (int(s.group(1)) if s else -1)
