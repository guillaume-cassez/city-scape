...
Le résultat principal est un **décalage entre training court et long**. À 10 epochs, la formulation conjointe C mène (78,17 vs 75,91 mIoU, Δ = +2,26 sur B en moyenne), cohérent avec la recette canonique Dice + boundary. À 160 epochs, l'image s'inverse : la variante boundary-only **D atteint la plus haute mIoU moyenne (81,69 ± 0,25) et le meilleur Boundary F1 (77,32 ± 0,13)**, tandis que les variantes Dice mènent sur le Trimap IoU (B 53,82 ± 0,36, C 53,87 ± 0,53). La significativité du mIoU utilise un **bootstrap apparié sur les images** des 500 images val comme test primaire pré-spécifié, le test t apparié par seed (n=3) servant de couche de robustesse : sous le bootstrap-images (Holm), D bat à la fois la baseline standard CE + Dice B (Δ = +0,59, p = 0,032) et le CE simple A (Δ = +0,40, p = 0,043), tandis que D > C n'atteint la significativité qu'au niveau seed (p = 0,042) ; l'effet est directionnellement cohérent sur les trois seeds, et les deux tests soutiennent conjointement chaque « D > {A, B, C} ». Les deux métriques de contour, en revanche, se séparent nettement le long de l'axe Dice avec une forte significativité : les variantes non-Dice (A, D) mènent le Boundary F1 (D − B = +0,93, A − B = +0,85 ; Holm p ≤ 0,011) tandis que les variantes Dice (B, C) mènent le Trimap IoU (B − D = +1,57, C − D = +1,62 ; Holm p ≤ 0,012). Le terme Dice échange la netteté des contours contre la cohérence régionale ; passé le coude de saturation, il n'aide ni ne pénalise la mIoU globale mais freine la convergence finale sur les grandes classes structurées.

**Contributions.** (1) Une ablation 2×2 reproductible de loss à 1024×2048 avec le mIoU officiel Cityscapes (estimateur validé bit-pour-bit contre `cityscapesscripts`), des IC 95 % Student-t et des tests de significativité appariés par seed corrigés par Holm sur trois métriques pour douze runs. (2) Une évidence empirique que les ablations courtes sont trompeuses sur cette tâche — à 10 epochs un pilote choisit C, mais à 160 epochs D dépasse significativement C (p = 0,007). (3) Une décomposition par classe montrant que D mène sur les grandes classes structurées (truck +5,29, wall +3,73, bus +2,37 mIoU vs B) tandis que B préserve les classes thin riches en signal (traffic light +2,10, train +1,10, traffic sign +0,86 mIoU vs D). (4) Un **filtre de consensus** par composantes connexes (veto variant-pair, adapté de BRATS) : l'appariement canonique **C⊘B** (la variante boundary-avec-Dice C vetoée par la baseline CE + Dice B — l'analogue BRATS direct) élague **−18,7 % des fragments parasites à coût mIoU nul et sans coût significatif sur la qualité de bord**, un gain de cohérence spatiale pur invisible au mIoU ; la matrice d'ablation complète des quatre appariements est rapportée. (5) Publication ouverte du code, des configs, des métriques par epoch des douze runs, et d'une page web compagnon avec figures interactives.
...
* **mIoU** : Intersection-over-Union moyenne sur les 19 classes à partir d'une unique matrice de confusion au niveau du dataset (label void exclu), calculée à pleine résolution. L'estimateur est validé comme bit-identique à la routine officielle `cityscapesscripts` (`tests/test_official_miou.py`), de sorte que le mIoU rapporté est la valeur officielle Cityscapes.
...
**Endpoint primaire pré-spécifié et protocole de significativité.** L'unique endpoint primaire pré-spécifié est le **mIoU** — la valeur officielle `cityscapesscripts`. Le **test de significativité primaire est un bootstrap apparié sur les images** des 500 images de validation : les images sont rééchantillonnées avec remise (B = 10 000 réplicats, seed fixe), le Δ mIoU au niveau dataset est recalculé par réplicat, l'IC 95 % est l'intervalle des percentiles 2,5/97,5, et la p-value bilatérale est $2 \cdot \min(\mathrm{frac}\,\Delta \le 0,\ \mathrm{frac}\,\Delta \ge 0)$. Ce test sonde la source de variance dominante sur un benchmark de 500 images — l'échantillonnage du jeu d'évaluation lui-même. Les métriques de contour (Boundary F1, Trimap IoU) et le fragment count sont secondaires ; l'IoU par classe et la dynamique epoch-10-vs-160 sont exploratoires.

Le **test t apparié par seed (n=3, Student) est l'analyse de robustesse**, rapporté à côté du test primaire pour sonder une source de variance *différente* — le seed aléatoire d'entraînement. Les résultats au niveau seed sont rapportés en moyenne sur trois seeds avec intervalle de confiance 95 % via la valeur critique de **Student** ($t_{0{,}975,\,df=2} = 4{,}303 \times \mathrm{SE}$ ; l'approximation normale 1,96 sous-estime l'intervalle d'un facteur ~2,2 à n=3). Le test t apparié par seed bloque sur le seed partagé et est plus puissant que la comparaison de chevauchement des barres d'IC, mais il est **explicitement sous-puissant à n = 3**. À l'intérieur de chaque métrique, les p-values par paires sont corrigées par Holm pour les comparaisons multiples dans les **deux** tests (α familial = 0,05).

> **Note de correction (cette révision).** Deux problèmes d'estimateur dans la v0.1.0 ont été trouvés après le premier brouillon du manuscrit et corrigés. (i) Le Boundary F1 et le Trimap IoU étaient calculés en appliquant une morphologie binaire à la *carte de labels multi-classes*, ce qui traite toute classe non nulle comme avant-plan — seul le contour route-vs-reste était donc mesuré, pas les frontières inter-classes que les métriques prétendent moyenner. Les deux sont désormais calculés par classe sur masques binaires, et **les douze runs ont été ré-évalués sur le val set** avec l'estimateur corrigé ; les chiffres de §5 sont les chiffres corrigés. Les valeurs absolues de contour bougent substantiellement par rapport à la v0.1.0 (Boundary F1 ≈ 58 → ≈ 77, Trimap ≈ 48 → ≈ 53) parce que la métrique moyenne désormais sur les 19 classes au lieu de route-vs-reste — et l'ordre *qualitatif* change aussi, de sorte que les chiffres de contour de la v0.1.0 ne doivent pas être cités. (ii) Les IC 95 % utilisaient 1,96 au lieu du facteur de Student et la significativité s'appuyait sur le chevauchement des barres d'IC ; les intervalles utilisent désormais $t_{0{,}975,df=2} = 4{,}303$ et les comparaisons utilisent des tests t appariés par seed corrigés par Holm. La mIoU rapportée est inchangée (elle n'a jamais dépendu du bug de contour) et est calculée par un estimateur **validé bit-pour-bit contre la routine officielle `cityscapesscripts`** (`tests/test_official_miou.py`).
...
IC = 95 % Student-t (df = 2). Les trois colonnes sont les estimateurs corrigés par classe, ré-évalués sur les 500 images du val set ; le mIoU égale la valeur officielle `cityscapesscripts` (validé bit-pour-bit). Le gras marque la meilleure moyenne par colonne (sur le Trimap, B et C sont statistiquement à égalité — Δ = 0,05, p = 0,71 — C est nominalement le plus haut).

Trois observations :

1. **D a la plus haute mIoU moyenne**, de +0,40 / +0,59 / +0,46 sur A / B / C. On rapporte les deux tests de significativité côte à côte — ils sondent deux sources de variance différentes (échantillonnage des images vs seed d'entraînement), donc aucun seul ne tranche toutes les paires.
   * **Primaire — bootstrap apparié sur les images (n = 500, B = 10 000, Holm) :** **D > B est significatif** (Δ = +0,59, Holm p = 0,032) et **D > A est significatif** (Δ = +0,40, Holm p = 0,043) ; **D > C n'est pas significatif après Holm** (Δ = +0,46, p brut = 0,023, Holm p = 0,092). A, B et C sont mutuellement non significatifs.
   * **Robustesse par seed — test t apparié par seed (n = 3, Holm) :** **D > C est significatif** (Δ = +0,46, t = 11,8, p brut = 0,007, Holm p = 0,042 — C a une très faible variance inter-seed) ; **D > A (p = 0,095) et D > B (p = 0,075) ne le sont pas** (sous-puissant à trois seeds, bien que les trois seeds favorisent D dans les deux cas).
   * **Synthèse :** D a la plus haute mIoU, et chaque paire « D > {A, B, C} » est soutenue par au moins un des deux tests — le test images établit D > A et D > B, le test seed établit D > C. Les deux tests sondent des sources de variance différentes (rééchantillonner le jeu d'évaluation de 500 images vs retirer le seed d'entraînement), donc on rapporte les deux plutôt que de cherry-pick celui qui flatte une paire donnée. L'affirmation de la v0.1.0 selon laquelle la borne basse d'IC de D dépasse les bornes hautes des autres reposait sur des intervalles seed à l'échelle 1,96 (trop serrés) ; l'énoncé honnête est que *D est la meilleure recette en moyenne, son avance sur la baseline standard B et sur le CE simple (A) est significative sous le bootstrap-images primaire, et son avance sur la variante conjointe C est significative sous le test au niveau seed*.
2. **Le Boundary F1 se sépare le long de l'axe Dice.** Les deux variantes *sans* Dice mènent — D (77,32) et A (77,24) — sur les variantes Dice C (76,50) et B (76,39). D − B = +0,93 et A − B = +0,85 sont significatifs (p ajusté par Holm = 0,011 et 0,010) ; D − A = +0,07 ne l'est pas (les deux meneurs sont à égalité). Le terme Dice *adoucit* mesurablement les contours prédits. Cette structure était invisible sous l'estimateur route-vs-reste de la v0.1.0, qui plaçait les quatre variantes à moins de 0,2 les unes des autres et conduisait à « aucune affirmation ».
3. **Les variantes Dice gagnent sur le Trimap IoU**, et l'effet est fortement significatif. B (53,82) et C (53,87) mènent les deux variantes non-Dice : B − D = +1,57, C − D = +1,62, B − A = +1,65, C − A = +1,70, toutes Holm-significatives (p ≤ 0,012) ; B et C sont à égalité (Δ = 0,05, p = 0,71), tout comme A et D (Δ = 0,08, p = 0,24). C'est l'image miroir du Boundary F1 : l'emphase régionale du Dice préserve la cohérence de blob près des contours au prix de la netteté des bords.

### 5.2 Dynamique de convergence — le retournement 10 vs 160 epochs

![Convergence](figures/fig_convergence.png)

*Figure 1 : mIoU par epoch (moyenne sur 3 seeds ; bande ombrée = IC 95 % Student-t). Le Boundary F1 / Trimap IoU par epoch ne sont pas montrés : seuls les checkpoints d'epoch 160 ont été conservés, de sorte que les métriques de contour corrigées par classe n'ont pas pu être recalculées aux epochs intermédiaires — le Tableau 1 donne les valeurs corrigées à epoch 160.*

À **epoch 10** la formulation conjointe **C est clairement la meilleure sur le mIoU** (la métrique sur laquelle le classement s'inverse plus tard) :

| Métrique | A | B | C | D |
|---|---|---|---|---|
| mIoU (10 ep) | 75,75 | 75,91 | **78,17** | 76,25 |

(Le Boundary F1 / Trimap par epoch sont omis — voir §4.2 : seul l'epoch 160 a été ré-évalué avec l'estimateur corrigé par classe.)

C mène B de **+2,26 mIoU** à epoch 10, un delta qui pousserait toute étude d'ablation courte à recommander la formulation conjointe. Cet écart précis **n'est pas significatif** individuellement à n=3 (p apparié = 0,10), gonflé par un seed à forte variance (par seed C−B = +1,15 / +1,84 / +3,79). Le signal robuste et *significatif* est le **retournement D-vs-C** : D est derrière C à epoch 10 (−1,92 mIoU) et le dépasse à epoch 160 (+0,46, p apparié = 0,007). À **epoch 50** les quatre variantes convergent dans une bande plus serrée (Δ < 1 mIoU) ; après epoch 100 **D prend la tête et y reste à partir d'epoch 110**. Le retournement est reproductible sur les trois seeds.

C'est l'observation centrale du papier : **une ablation à 10 epochs sur cette tâche choisit un autre vainqueur que le run convergé** — significativement sur l'axe D/C, et directionnellement sur C/B. Le terme Dice fournit une régularisation en début d'entraînement qui accélère la convergence (visible dans la montée de mIoU entre epochs 4 et 10) mais ne se traduit pas en avantage long-training sur la métrique globale. Le terme boundary, lui, prend plus d'epochs pour s'intégrer au signal de gradient — le champ SDT fournit un gradient distribué faible qui a besoin de plus de steps pour déplacer la frontière de décision — mais finit par produire une mIoU et un Boundary F1 plus élevés une fois convergé.
...
Les IC 95 % Student-t induits par les seeds varient largement entre métriques et variantes. Parmi les métriques globales, C a l'IC mIoU le plus serré (±0,21) et D le Boundary F1 le plus serré (±0,13) ; les IC de contour les plus larges sont ceux de C (±0,54 Boundary F1, ±0,53 Trimap). Au niveau classe, **truck** sous la variante B est la plus volatile — écart-type inter-seeds de 4,40 points IoU (vs 2,55 std pour D, 0,68 pour C, 0,78 pour A). Avec truck apparaissant dans seulement **80 des 500 images val**, l'emphase régionale du Dice amplifie les fluctuations sur les classes à faible support.

### 5.5 Filtrage par consensus — élagage des fragments parasites

La décomposition par classe (§5.3) montre que D et B sont *complémentaires* : D mène sur les grandes classes structurées, B sur les classes thin riches en signal. Cela invite à une étape de consensus. On adapte le **filtre consensus** par composantes connexes (CC) de nos travaux BRATS, où une segmentation « généraliste » est vetoée par une « spécialiste » : par classe, toute composante connexe du généraliste sans recouvrement de même classe dans le veto est retirée, ce qui élague les fragments hallucinés sans coût sur le recouvrement de région.

**Quel appariement est l'analogue BRATS ?** Dans nos travaux BRATS, le veto est la **baseline standard du domaine** (`DC_and_CE`, c.-à-d. Dice + CE — la loss nnU-Net par défaut), et le généraliste qu'elle élague (DistMap) *garde lui aussi Dice + CE*. La transcription Cityscapes fidèle doit donc utiliser la **baseline CE + Dice** comme veto. Parmi nos quatre variantes, cette baseline est **B** (CE + Dice) — c'est la baseline standard de segmentation et elle égale la loss nnU-Net par défaut `DC_and_CE` utilisée par les travaux BRATS ; A (CE seule) et D (CE + Kervadec, sans Dice) sont des points d'ablation *sur l'axe Dice* spécifiques à Cityscapes, **sans équivalent BRATS** (nnU-Net inclut toujours le Dice). L'analogue strict de la règle BRATS est donc la variante boundary-avec-Dice **C** (qui, comme DistMap, garde Dice + CE) vetoée par la baseline Dice + CE **B** : le **consensus canonique est C⊘B**. (L'appariement D⊘B rapporté en v0.2.1 est une trouvaille 2D spécifique à Cityscapes — son veto B est la baseline, mais son généraliste D abandonne le Dice, donc ce n'est *pas* la transcription BRATS directe ; on le rapporte ci-dessous en contraste.)
...
|---|---|---|---|
| **C⊘B (canonique)** | 81,24 | +0,010 pp | **−18,7 %** |
| D⊘B (trouvaille v0.2.1) | 81,65 | −0,033 pp | −18,6 % |
| C⊘A | 81,31 | +0,085 pp | −8,5 % |
| D⊘A | 81,82 | +0,137 pp | −15,6 % |

Deux faits structurels. Premièrement, **chaque appariement est mIoU-neutre** (|Δ mIoU| ≤ 0,14 pp) : le veto élague les fragments sans toucher au recouvrement de région, par construction. Deuxièmement, **le veto B (= baseline Dice + CE) élague ≈ 2× plus de fragments que le veto A** (C⊘B −18,7 % vs C⊘A −8,5 % ; D⊘B −18,6 % vs D⊘A −15,6 %) : la baseline Dice B est un masque de veto plus fort et plus propre, ce qui confirme que le veto BRATS-fidèle est la baseline B plutôt que l'ablation CE-seule A.

**Consensus canonique C⊘B (caractérisation complète, fusionné vs primaire C, apparié n = 3).** C'est l'analogue BRATS strict (un généraliste Dice + CE vetoé par la baseline Dice + CE). Contre le primaire C il donne :

| Métrique | Δ (fusionné − C) | p (apparié par seed, n = 3) | bootstrap-images |
|---|---|---|---|
| mIoU | +0,010 pp | 0,747 | Δ = −0,01, p = 0,86 → **neutre** |
| fragments | −18,7 % (−111/img) | 0,007 | — |
| Boundary F1 | −0,104 pp | 0,009 | négligeable (< 0,1 pp) |
| Trimap IoU | +0,058 pp | 0,028 | négligeable (< 0,1 pp) |

Le mIoU est confirmé neutre par *les deux* tests — le test t apparié par seed (p = 0,747) et le bootstrap apparié sur les images (Δ = −0,01, p = 0,86). Les décalages de Boundary F1 et Trimap IoU sont statistiquement détectables à n = 3 (p = 0,009 et 0,028) mais **pratiquement négligeables** — tous deux sous 0,1 pp. C⊘B est donc un **nettoyage de cohérence spatiale pur** : il retire ≈ 19 % des fragments (composantes connexes) à **coût mIoU nul et coût qualité-de-bord nul significatif**. C'est l'analogue BRATS propre — neutre sur le recouvrement de région, le gain vivant entièrement sur la fragmentation, propriété à laquelle le mIoU au niveau dataset est aveugle (c'est exactement pourquoi la métrique de fragment count est rapportée à ses côtés ; sur BRATS la règle analogue était Dice-neutre mais améliorait le HD95 de bord).

**Contraste D⊘B (la trouvaille v0.2.1).** D est la meilleure variante seule, donc D⊘B est un appariement naturel, mais son veto B est la baseline alors que son généraliste D abandonne le Dice — ce n'est donc pas la transcription BRATS directe. Il donne le **même élagage et la même mIoU-neutralité** (fragments −18,6 %, −146/img, p = 0,007 ; mIoU Δ = −0,033 pp, p seed = 0,50, bootstrap-images Δ = +0,03, p = 0,66) **mais un décalage notable du caractère Dice** : Boundary F1 −0,691 pp et Trimap IoU +0,997 pp (les deux p < 0,001). Le mécanisme : réassigner les fragments de D aux labels de B *Dice-ifie* la sortie, parce que D (sans Dice) et B (avec Dice) sont aux extrémités opposées de l'axe Dice (§5.1) — le masque fusionné hérite du caractère région-cohérent et contour-adouci de B sur les pixels réassignés. C⊘B (les deux variantes du côté Dice) n'a pas cet effet de bord. **C⊘B est donc le consensus *pur* ; D⊘B confond le nettoyage de fragments avec un décalage sur l'axe Dice.**
...
### 6.2 Boundary F1 vs Trimap IoU — le compromis Dice

Les deux métriques de contour se séparent le long de l'axe Dice, et *les deux* séparations sont significatives (§5.1). Le Trimap IoU est une IoU restreinte aux pixels à 3 px d'un contour vérité terrain : il pénalise les faux positifs *hors* de la vraie région (expansion de contour) et les faux négatifs *dedans* (rétraction). Le Dice optimise un ratio d'overlap régional, qui tire la prédiction vers le bulk de la région — ce qui garde la bande proche du contour cohérente (Trimap IoU plus élevé pour B et C) mais arrondit les détails fins de contour (Boundary F1 plus bas). Les deux variantes sans Dice (A = CE, D = CE + boundary) gardent des bords plus nets et mènent donc le Boundary F1 — D le plus haut, le gradient SDT alignant explicitement les contours — au prix de ≈ 1,6 points de Trimap. Les métriques corrigées par classe rendent ce compromis à la fois visible et significatif ; l'estimateur route-vs-reste de la v0.1.0 ramassait le Boundary F1 à moins de 0,2 sur les quatre variantes et le cachait.

### 6.3 Prises actionnables

* **Pour un modèle Cityscapes déployé** : D (CE + Kervadec, $\lambda_b = 0,2$) est le défaut pragmatique. C'est la plus simple des quatre (pas de plumbing Dice, pas d'hyperparamètre), a la plus haute mIoU moyenne — significativement au-dessus de la variante conjointe C, et à égalité avec le CE simple (A) à ce nombre de seeds — et se comporte de manière prévisible sur les grandes classes structurées. Là où la cohérence régionale près des contours compte plus que la netteté des bords, une variante Dice (B ou C) est préférable — B et C mènent le Trimap IoU de ≈ +1,6 (significatif) — et B protège en plus les classes thin riches en signal (traffic light, traffic sign).
...
* **Trajectoires de contour par epoch non ré-évaluées.** Les Boundary F1 / Trimap corrigés par classe ont été recalculés à epoch 160 pour les douze runs (Tableau 1) ; les checkpoints aux epochs intermédiaires n'ont pas été conservés, de sorte que la figure de convergence (§5.2) ne montre que la mIoU.
* **Faible puissance statistique (n = 3).** Les écarts < 0,6 mIoU (D vs A, D vs B) sont directionnellement cohérents mais non significatifs à trois seeds. Un re-run à cinq seeds est le moyen le moins cher de les trancher. Les résultats significatifs (D > C sur mIoU ; les variantes Dice B, C > A, D sur Trimap ; A, D > B sur Boundary F1 ; le retournement D/C) ne sont pas affectés.
* **Consensus entièrement caractérisé ; caveat n = 3 maintenu.** §5.5 rapporte désormais l'effet du veto variant-pair sur les quatre métriques (mIoU, fragments, Boundary F1, Trimap — l'analogue du HD95 de BRATS) pour la matrice complète des appariements : le C⊘B canonique est neutre sur la qualité de bord (décalages Boundary F1 / Trimap < 0,1 pp), tandis que D⊘B porte un décalage mesurable sur l'axe Dice (Boundary F1 −0,69 pp, Trimap +1,00 pp). La mIoU-neutralité est confirmée par le bootstrap-images et le test au niveau seed ; les deltas de contour sont détectables mais, mesurés à trois seeds, partagent le même caveat de faible puissance que le reste de l'analyse au niveau seed.
...
* **Heads « drivable area » / « free-space »** qui alimentent une grille d'occupation : une variante Dice (B ou C), dont l'avantage ≈ +1,6 Trimap IoU sur les variantes non-Dice préserve la cohérence du blob et évite l'overshoot sur la classe voisine.
...
On fournit une ablation 2×2 reproductible de l'espace de design des losses CE / Dice / boundary Kervadec pour la segmentation sémantique pleine résolution sur Cityscapes. À 160 epochs avec trois seeds par variante, la variante boundary-only **D (CE + Kervadec)** atteint la plus haute mIoU moyenne (81,69 ± 0,25) et le meilleur Boundary F1 (77,32 ± 0,13) ; sous un test apparié par seed elle dépasse significativement la variante conjointe C (p = 0,007) — la formulation qu'un pilote à 10 epochs aurait choisie — tout en restant statistiquement à égalité avec le CE simple (A) à ce nombre de seeds. Les variantes Dice (B, C) gardent une avance significative sur le Trimap IoU (≈ +1,6 sur A et D), reflétant leur meilleure cohérence intra-région près des contours — l'image miroir de l'ordre du Boundary F1.
