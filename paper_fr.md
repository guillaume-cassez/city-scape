...
Toutes les métriques sont rapportées en moyenne sur trois seeds avec intervalle de confiance 95 % via la valeur critique de **Student** ($t_{0{,}975,\,df=2} = 4{,}303 \times \mathrm{SE}$ ; l'approximation normale 1,96 sous-estime l'intervalle d'un facteur ~2,2 à n=3). Les comparaisons par paires utilisent un test t apparié par seed, qui bloque sur le seed partagé et est plus puissant que la comparaison de chevauchement des barres d'IC ; à l'intérieur de chaque métrique, les six p-values par paires sont corrigées par Holm pour les comparaisons multiples (α familial = 0,05).
...
1. **D a la plus haute mIoU moyenne**, de +0,40 / +0,59 / +0,46 sur A / B / C. Mais la significativité ne suit pas automatiquement à n=3. Un test t apparié par seed ne rend significatif que **D > C** (Δ = +0,46, t = 11,8, p brut = 0,007, p ajusté par Holm = 0,042 — C a une très faible variance inter-seed) ; **D > A (p = 0,095) et D > B (p = 0,075) ne sont pas significatifs**, bien que les trois seeds favorisent D dans les deux cas. L'affirmation de la v0.1.0 selon laquelle la borne basse d'IC de D dépasse les bornes hautes des autres reposait sur des intervalles à l'échelle 1,96 (trop serrés) ; avec le facteur t correct, les IC mIoU de A / B / D se chevauchent. L'énoncé honnête : *D est la meilleure recette en moyenne et bat significativement la variante conjointe C, mais reste statistiquement indistinguable du CE simple (A) à ce nombre de seeds*.
...
Le consensus est un **veto variant-pair** : D (généraliste) vetoé par B (spécialiste), ciblant les gains grandes-classes de D tout en laissant B vetoer les fragments parasites — l'analogue direct de la règle BRATS Baseline-vetoe-DistMap, évalué par seed. On rapporte aussi un **fragment count** (composantes connexes par classe), proxy de cohérence spatiale indépendant du mIoU que le veto ne peut que baisser.

**Résultats (val Cityscapes, 3 seeds).** Sur la variante D, le veto retire **18,6 % des fragments** (composantes connexes) (782,6 → 636,8 par image ; cohérent sur les 3 seeds, p apparié = 0,008) à **coût mIoU nul** (81,69 → 81,65 ; Δ = −0,03 pp, p apparié = 0,50). Il élague les fragments parasites sans toucher au recouvrement de région — son but, et le même nettoyage spatial mIoU-neutre observé sur BRATS (où la règle analogue était Dice-neutre mais améliorait le HD95 de bord). Le veto est donc un outil de **cohérence spatiale**, pas un gain de mIoU. Le filtre, la métrique de fragment count, le script d'évaluation (`scripts/evaluate_consensus.py`) et une suite de tests synthétiques (19/19) sont publiés avec le code (`src/postprocessing/consensus.py`).
...
* **Consensus caractérisé sur mIoU + fragments seulement.** §5.5 rapporte le Δ mIoU et le Δ fragment count du veto variant-pair sur le val ; son effet sur la qualité de bord (Boundary F1 / Trimap, l'analogue du HD95 de BRATS) n'est pas encore mesuré.
